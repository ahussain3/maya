...
var color = d3.scaleLinear().domain(priceBands).range(d3.schemeOrRd[9]);

// DRAW THE KEY / LEGEND
var linear = d3.scaleLog().domain([0, 10]).range(["rgb(46, 73, 123)", "rgb(71, 187, 94)"]);

svg.append("g").attr("class", "legend-box").append("g").attr("class", "legend").attr("transform", "translate(20,20)");

var legendLinear = d3.legendColor().labels(priceBands.map(function (value) {
    return "£" + d3.format(".2s")(value);
})).cells(priceBands).shapeWidth(30).orient('vertical').scale(color);
